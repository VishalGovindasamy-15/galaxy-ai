...
class TestBuildRestAPI:
    ...
    async def test_build_rest_api(self) -> None:
        ...
        # Plan: 4 files with dependency chain
...
class TestCrashAndRecover:
    ...
    def test_crash_and_recover(self, tmp_path: Path) -> None:
        ...
        cp.create(state, label="pre_crash")
...
class TestPauseSwapModelResume:
    ...
    async def test_pause_swap_model_resume(self) -> None:
        ...
        worker_config = router.get_model_for_tier(AgentTier.WORKER)
        assert worker_config.model == "mock-7b"

        # Swap model
        from galaxy.core.config import ModelConfig
        new_config = ModelConfig(provider="mock", model="mock-14b")
        router.swap_model(AgentTier.WORKER, new_config)
...
class TestMultiWorkerParallel:
    ...
    async def test_multi_worker_parallel(self) -> None:
        ...
        # Plan: 5 independent tasks (no deps = max parallelism)
        tasks = [
            Task(description=f"File {i}", file_path=f"file_{i}.py")
            for i in range(5)
        ]
        await orch.plan(tasks)

        # Execute with 3 workers max
...
class TestToolIntegrationE2E:
    """E2E: Worker uses tools to write and validate files."""
...
class TestFullLifecycle:
    """E2E: Complete lifecycle — boot → plan → execute → checkpoint → shutdown."""
...
class TestEventPropagation:
    """E2E: Events flow correctly through the entire system."""
    ...
    async def test_events_flow_through_system(self) -> None:
        ...
        # Should have orchestrator + agent events
